...
const Inbox = ({calls, url, callUpdate}) => {
    ...
    async function archive(call_id){
    ...
    <div>
        <Accordion>
        {calls.map(i => {
                if (calls.length > 0 && i.is_archived === false) {
                    return (
                        <Accordion.Item eventKey={i.id} key={i.id} >
                            <Accordion.Header> 
                                <Col xs={9}> {i.direction === 'outbound' ? 
                                <div><BsTelephoneOutbound /> Calling: {i.to}</div>
                                :
                                <div><BsTelephoneInbound /> Call From: {i.from}</div>} </Col>
                                <Col id="call_type" xs={2}>{i.call_type.charAt(0).toUpperCase()+ i.call_type.slice(1)}</Col> </Accordion.Header>
                            <Accordion.Body id={i.call_type}> {/* ID here gives the accordion body it's color depending on if it's Answered, Missed or Voicemail*/}
                                ...
                                <Button variant="warning" onClick={() => archive(i.id)}>Archive</Button> 
                            </Accordion.Body>
                        </Accordion.Item>
